...
#' known.variants <- c("8999 T>C","4818 G>A","11126 G>A","7228 A>G","5920 G>A","15734 G>A","14804 G>A")
#' known.subset <- pullcountsVars(LudwigFig7.Counts, known.variants)
...
#' LudwigFig7.plot <- plotTree(LudwigFig7)
#'@return Returns TRUE only used for generating a PostScript tree image of the putative mutation tree
#'@export
plotTree <- function(mutcalls, file = "mytree.ps") {
    ...
    system(sprintf("dot -Tps %s > %s", tmp, file),wait = TRUE)
    file.remove(tmp)
}
